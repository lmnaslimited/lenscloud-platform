...
class Site(Document):
    ...
    def validate(self):
        ...
        if self.subscription:
            ...
            if subscription.customer != self.customer or subscription.plan != self.plan or subscription.region != self.region:
                frappe.throw(_("Site Customer, Plan, and Region must match its Subscription."))
            policy = environment_policy(subscription, self.environment)
            self.site_control_profile = policy["site_control_profile"]
            self.policy_hash = subscription.policy_hash
            duplicate = frappe.db.exists("Site", {
            ...
            if duplicate:
                frappe.throw(_("Subscription already has an active Site for Environment {0}.").format(self.environment))
        if self.region:
