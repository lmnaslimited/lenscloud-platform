...
class Customer(Document):
	...
	# 	endpoint = f"{support_url.rstrip('/')}/api/resource/Customer"
	...
	# 		frappe.log_error(
	# 			title=f"Customer sync failed: {self.name}",
	# 			message=f"Failed to sync Customer {self.name} to {support_url}.\nError: {e}",
	# 		)
	# 		return
	# 	# Extract the created ticket ID from Frappe's standard response format: {"data": {"name": "..."}}
	# 	customer_id = response_data.get("data", {}).get("name")
	# 	if customer_id:
	# 		frappe.db.set_value(
	# 			"Customer",
	# 			self.name,
