...
Platform consumes the INF-010 Kubernetes Job/ConfigMap contract through the Python Kubernetes API only. The first supported command path is `bench_test.status`, which validates policy/target/args, creates the request ConfigMap and labelled Job, watches Job/Pod status, parses a sanitized termination summary, records an Orchestration Action Log, and deletes the temporary Job/ConfigMap after evidence capture. Contracted commands whose runner implementation is pending return `Unsupported` with `COMMAND_UNSUPPORTED`; Platform must not present those as live enforcement. Infra verification is complete at revision `dcd94d8`; Platform live smoke succeeded in action log `ORCH-2026-00137` and verified temporary Job/ConfigMap cleanup.
