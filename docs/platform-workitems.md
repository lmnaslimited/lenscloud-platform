...
| Release Group site app install contract | Convert Release Group included apps from Table MultiSelect to a child table with install-at-site-creation and install-sequence flags; Platform must include selected apps in Site creation manifests and Infra must install them in sequence during new Site bootstrap | Pending | 2026-07-09 |  |
...
| Release Group app refresh and existing Site install | When a Release Group gains apps, Benches refresh available apps and Platform/customer portal users can install eligible apps to existing Sites through a governed install action backed by Infra command execution | Pending | 2026-07-13 |  |
