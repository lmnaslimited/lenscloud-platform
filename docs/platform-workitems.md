...
| Launch E2E incident tracking | Every Platform/customer E2E failure is recorded in `docs/incidents/e2e-incident-tracker.md` with severity, owner, evidence, affected scenario, action log/reference IDs, fix status, and retest result; first-class Incident DocType remains a later decision | In Progress | 2026-07-02 |  |
| Customer provisioning retry and paused-apply UX | Customer Free Plan provisioning distinguishes real apply, paused/dry-run, failed, and ready states; customer can retry an existing reserved Site after Platform opens the controlled apply window without seeing runtime internals | In Progress | 2026-07-02 |  |
