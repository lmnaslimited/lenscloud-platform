...
| Release Group bootstrap app install contract | Release Group included apps use a child table with install-at-site-creation and install-sequence flags; Platform derives bootstrap app payloads from the Site Bench Release Group, excludes frappe, and runs Infra app-aware Site bootstrap jobs with the Release Group runtime digest; live Free Plan is blocked by `LC-E2E-20260717-001` until Infra returns the app install root cause | Blocked | 2026-07-09 |  |
| Site bootstrap app install incident | Customer Free Plan Site `tharahub.cloud.lmnaslens.com` reaches runtime/HTTPS Ready but Release Group default app install fails; Platform timeline/context now shows the default-app step as failed and Infra has a handoff to return root cause/details | Blocked | 2026-07-17 |  |
| Bench next-release update gate | When a new Release is released for a Release Group, Platform can set Bench next release for Benches in that group; Site upgrade scheduling stays manual but requires upgrade tested, tested on, and tested by before moving the Site to Scheduled; Bench update can run only after every Site on the Bench is Scheduled | Pending | 2026-07-13 |  |
