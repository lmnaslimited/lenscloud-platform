...
| CUA setup wizard runner integration | Platform wires INF-021 `site_setup.status` and `site_setup.complete` through the existing Python Kubernetes API Bench Command path, records customer-safe setup progress, keeps args non-secret, and is ready for controlled Free Plan live E2E after Platform commit `c520b5a` removed the live-verification block | In Progress | 2026-07-06 |  |
