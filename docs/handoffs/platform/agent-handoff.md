...
Metadata-driven center editor: Platform forms now load LensCloud DocType field order and render Tab Break, Section Break, Column Break, Table, and Table MultiSelect fields. Release Group Included Apps is now a full child table with App value help, Install At Site Creation, and Install Sequence columns. Wide child grids keep row identity/primary/actions fixed and scroll remaining columns horizontally.

Release/Bench app-install contract: `Release Group.included_apps` now uses the `Release Group Apps` child table. New Site creation must pass rows where `install_at_site_creation` is checked to Infra in ascending `install_sequence`, and Infra must install them during Site bootstrap. Upcoming gaps: Bench `next_release` must be updatable when a new Release is released for the same Release Group; Site `upgrade_state = Scheduled` must be gated by `upgrade_tested`, `tested_on`, and `tested_by`; Bench update must require every Site on the Bench to be scheduled; and Release Group app additions must refresh Bench/Site available apps so Platform/customer users can install eligible apps to existing Sites. Infra handoff prompt: `docs/handoffs/infra/release-group-app-install-and-bench-upgrade-20260713.md`.
