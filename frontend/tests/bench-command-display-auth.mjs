...
try {
	...
	await page.getByRole('option', { name: 'maintenance_mode.status', exact: true }).click()
	await page.getByLabel('Args JSON').fill('{}')
	await page.getByLabel('Timeout seconds').fill('120')
	await page.getByLabel('Reason').fill('Authenticated UI verification of Bench Command display contract')
	...
	await resultCard.getByText('Maintenance mode:', { exact: true }).waitFor()
	await resultCard.getByText(/\bOff\b|\bOn\b/).waitFor()
	if (await resultCard.getByText(/password|secret|token|private key/i).count()) throw new Error('Secret-like text appeared in Bench Command display result card.')
	if (errors.length) throw new Error(`Bench Command display browser errors: ${errors.join('; ')}`)
	console.log(`Authenticated Bench Command display passed for ${site}.`)
} finally {
	await context.close()
	await browser.close()
}
