...
try {
	...
	await saveAndWait(page, editor, original)
		...
		await releaseEditor.getByRole('button', { name: 'Apps', exact: true }).click()
		await releaseEditor.getByRole('heading', { name: 'Included Apps', exact: true }).waitFor()
		...
		await releaseEditor.getByRole('columnheader', { name: /App/ }).waitFor()
		await releaseEditor.getByRole('columnheader', { name: /Install At Site Creation/ }).waitFor()
		await releaseEditor.getByRole('columnheader', { name: /Install Sequence/ }).waitFor()
		const releaseGroupApp = releaseEditor.getByLabel('App', { exact: true }).first()
		await releaseGroupApp.waitFor()
		if (await releaseGroupApp.inputValue() !== 'erpnext') throw new Error('Release Group included app row did not load erpnext.')
		await releaseGroupApp.click()
		await page.getByRole('option', { name: /erpnext/i }).first().waitFor()
		await page.keyboard.press('Escape')
		await releaseEditor.getByRole('button', { name: 'Remove row' }).first().click()
		...
		for (let attempt = 0; attempt < 50; attempt += 1) {
			if (await releaseGroupApp.inputValue() === 'erpnext') break
			await page.waitForTimeout(100)
		}
		if (await releaseGroupApp.inputValue() !== 'erpnext') throw new Error('Discard did not restore the Release Group included app row.')

		if (errors.length) throw new Error(`Browser errors: ${errors.join('; ')}`)
		console.log('Authenticated metadata editor Playwright passed: sections, compact fixed columns, horizontal scroll, child persistence, Release Group app install flags, and rename-only autoname fields.')
} finally {
	await context.close()
	await browser.close()
}
