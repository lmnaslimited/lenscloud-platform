	...
	<div class="flex h-screen overflow-hidden bg-surface-white text-ink-gray-9">
		<aside class="hidden w-60 shrink-0 flex-col border-r border-outline-gray-2 bg-surface-menu-bar lg:flex">
			...
			<div class="min-h-0 flex-1 overflow-y-auto px-2 py-2">
				<nav v-for="group in navGroups" :key="group.heading" class="mb-1">
			...
			<div class="border-t border-outline-gray-2 px-3 py-2"><div class="flex items-center justify-between gap-2"><span class="truncate text-xs text-ink-gray-5">{{ session.user }}</span><Badge class="bg-surface-gray-2 text-ink-gray-6">{{ currentScope }}</Badge></div></div>
		</aside>
