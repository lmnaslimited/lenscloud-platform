...
export const platformResources = [
	...
	},
	{
		...
		summaryFields: [
			...
			{ key: 'bench_command_runner_contract_status', label: 'Runner contract', readOnly: true, ...selectField(['Unknown', 'Synced', 'Failed']) },
		],
		detailFields: [
			...
			{ key: 'bench_command_runner_image', label: 'Bench Command Runner Image', readOnly: true },
			{ key: 'bench_command_runner_contract_status', label: 'Runner Contract Status', readOnly: true, ...selectField(['Unknown', 'Synced', 'Failed']) },
			{ key: 'bench_command_runner_contract_checked_on', label: 'Runner Contract Checked On', readOnly: true },
			{ key: 'bench_command_runner_contract_error', label: 'Runner Contract Error', readOnly: true },
		],
		actions: [
			{
			...
			{
				key: 'sync-bench-runner-contract',
				label: 'Sync Bench Runner Contract',
				icon: RefreshCcw,
				description: 'Read Infra accepted Bench Command runner digest from the Cluster contract ConfigMap and store it on this Cluster.',
				backendSupported: true,
				method: 'lenscloud.api.bench_command.sync_cluster_bench_command_runner_contract',
				paramsFromRecord: { cluster: 'name' },
				fields: [],
			},
			{
				key: 'validate-bench-runner-contract',
				label: 'Validate Bench Runner Contract',
				icon: ShieldAlert,
				description: 'Server-side dry-run a generic Bench Command Job with the synced runner digest so stale digest admission failures are visible before Site setup or OAuth commands run.',
				backendSupported: true,
				method: 'lenscloud.api.bench_command.validate_cluster_bench_command_runner_contract',
				paramsFromRecord: { cluster: 'name' },
				fields: [],
			},
			{
	...
	},
	{
		...
		actions: [
			...
			{ key: 'run_setup_default', label: 'Run Setup Default', icon: RefreshCcw, description: 'Execute setup wizard using information in the Setup Data JSON collected during site provisioning.', backendSupported: true, method: 'lenscloud.api.bench_command.run_site_setup_complete', paramsFromRecord: { site: 'name', args: 'setup_data_json' }, fields: [{ key: 'timeout_seconds', label: 'Timeout seconds', type: 'number', default: 900 }] },
			...
			},
			{
				...
				paramsFromRecord: { site: 'name' },
				fields: [
					{ key: 'command', label: 'Command', type: 'select', default: 'bench_test.status', options: ['bench_test.status', 'maintenance_mode.status', 'maintenance_mode.enable', 'maintenance_mode.disable', 'scheduler.enable', 'scheduler.disable', 'developer_mode.status', 'developer_mode.enable', 'developer_mode.disable', 'site_config.get', 'site_config.set', 'site_config.unset', 'cors.allowlist.get', 'cors.allowlist.update', 'backup.status', 'site_setup.status', 'site_setup.complete', 'oauth.status', 'backup.create', 'restore.preview', 'restore.execute', 'restore.status', 'bench_test.trigger', 'latp.status', 'latp.trigger'] },
	...
	},
	{
		...
		icon: LifeBuoy,
		listHelp: 'Customer-reported issues linked to Customer Member, Subscription, and Site, with severity, triage status, and helpdesk sync state.',
		creatable: true,
		editable: true,
		lifecycleFields: [
			{ key: 'customer_member', label: 'Customer Member', required: true, ...linkField('Customer Member', ['customer', 'user', 'member_role', 'status']) },
			...
			{ key: 'category', label: 'Category', ...selectField(['Issue', 'Service Request', 'Change Request']) },
			{ key: 'status', label: 'Status', default: 'Open', ...selectField(['Open', 'Resolved', 'Closed']) },
			{ key: 'severity', label: 'Severity', ...selectField(['High', 'Low', 'Medium']) },
			{ key: 'summary', label: 'Summary', type: 'textarea' },
			{ key: 'description', label: 'Description', type: 'textarea' },
		],
		summaryFields: [
			{ key: 'name', label: 'Ticket' },
			{ key: 'customer_member', label: 'Customer Member', linkPrefix: '/platform/customer-members/', ...linkField('Customer Member', ['customer', 'user']) },
			{ key: 'customer', label: 'Customer', linkPrefix: '/platform/customers/', ...linkField('Customer', ['first_name', 'last_name']) },
			...
			{ key: 'category', label: 'Category', ...selectField(['Issue', 'Service Request', 'Change Request']) },
			{ key: 'status', label: 'Status', ...selectField(['Open', 'Resolved', 'Closed']) },
			{ key: 'severity', label: 'Severity', ...selectField(['High', 'Low', 'Medium']) },
			{ key: 'helpdesk_ticket_id', label: 'Helpdesk ID' },
		],
		detailFields: [
			{ key: 'name', label: 'Ticket ID' },
			{ key: 'customer_member', label: 'Customer Member', linkPrefix: '/platform/customer-members/', ...linkField('Customer Member', ['customer', 'user', 'member_role', 'status']) },
			{ key: 'customer', label: 'Customer', readOnly: true, linkPrefix: '/platform/customers/', ...linkField('Customer', ['first_name', 'last_name']) },
			{ key: 'external_customer_id', label: 'External Customer ID', readOnly: true },
			{ key: 'email', label: 'Email', readOnly: true, ...linkField('User', ['full_name', 'email']) },
			{ key: 'subscription', label: 'Subscription', linkPrefix: '/platform/subscriptions/', ...linkField('Subscription', ['customer', 'plan', 'status']) },
			{ key: 'helpdesk_ticket_id', label: 'Helpdesk Ticket ID', readOnly: true },
			{ key: 'site', label: 'Site', linkPrefix: '/platform/sites/', ...linkField('Site', ['title', 'subdomain']) },
			{ key: 'category', label: 'Category', ...selectField(['Issue', 'Service Request', 'Change Request']) },
			{ key: 'status', label: 'Status', ...selectField(['Open', 'Resolved', 'Closed']) },
			{ key: 'severity', label: 'Severity', ...selectField(['High', 'Low', 'Medium']) },
			{ key: 'source', label: 'Source', ...selectField(['Customer']) },
			{ key: 'orchestration_action_log', label: 'Orchestration Action Log', readOnly: true, linkPrefix: '/platform/orchestration-logs/', ...linkField('Orchestration Action Log') },
